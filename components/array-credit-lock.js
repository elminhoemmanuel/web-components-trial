...
class ArrayCreditLock extends HTMLElement {
    ...
    getDate(given){
        let year = new Date(given).getFullYear();
        let month = new Date(given).getMonth();
        let day = new Date(given).getDay();
        let time = this.formatAMPM(new Date(given));
        const timezone = new Date(given).toLocaleTimeString('en-us',{timeZoneName:'short'}).split(' ')[2];
        return `${year}- ${month} - ${day < 10 ? `0${day}` : day} ${time} ${timezone}`;
    }

    renderInitial() {
        let initialList = this.shadow.querySelector('#initial-list-holder');
        for (let i = 0; i < this.initial.length; i++) {
            let li = document.createElement('li');
            li.setAttribute("class", "history-list");
            li.innerHTML = `<span class="date">${this.getDate(this.initial[i].date)}</span>
                        ...
                        <span class="lock">${this.initial[i].active ? "Unlocked" : "Locked"}</span>
                        </div>`
    ...
    renderAll() {
        let list = this.shadow.querySelector('#list-holder');
    ...
    toggler(item1, item2){
        ...
        let showLess = this.shadow.querySelector("#show-less");
        let smallList = this.shadow.querySelector(item1);
        smallList.style.display !== "none" ? smallList.style.display = "none" : smallList.style.display = "block"
        let largeList = this.shadow.querySelector(item2);
        largeList.style.display !== "none" ? largeList.style.display = "none" : largeList.style.display = "block"

        let hideText = this.shadow.querySelector(".hide-history");
        let showText = this.shadow.querySelector(".show-history");

        smallList.style.display === "block" ? hideText.style.display = "block" : hideText.style.display = "none"        
        smallList.style.display === "none" ? showText.style.display = "block" : showText.style.display = "none"        
        largeList.style.display === "block" ? hideText.style.display = "block" : hideText.style.display = "none"        
        largeList.style.display === "none" ? showText.style.display = "block" : showText.style.display = "none"       
        
        smallList.style.display === "none" ? showAll.style.display = "none" : showAll.style.display = "block"
        largeList.style.display === "none" ? showAll.style.display = "none" : showAll.style.display = "block"
    }
    ...
    connectedCallback() {
        if (this.url) {
            ...
            fetch(`${this.url}`)
                ...
                .then((data) => {
                    this.history = data.map((item, x) => {
                        return {
                            ...item,
                            id: x + 1
                        }
                    })
                    this.initial = this.history.filter(item => item.id <= 4)
                    this.renderInitial();
                    this.renderAll();
                    this.shadow.querySelector("#show-all").innerHTML = `Show All (${this.history.length})`
        ...
        let hider = this.shadow.querySelector("#hide-history");
        hider.addEventListener("click", ()=>{
            this.toggler("#initial-list-holder", "#list-holder")
        })

        let hider2 = this.shadow.querySelector("#show-history");
        hider2.addEventListener("click", ()=>{
            this.toggler("#initial-list-holder", "#list-holder")
        })
          ...
          <section class="credit-freeze-center">
            <ul>
              ...
              >
                ...
                <div>
                  <p  class="history-title hide-history" id="hide-history" >Hide lock history</p>
                  <p  class="history-title show-history" id="show-history" style="display:none;">Show lock history</p>
                  <ul id="initial-list-holder">

                  </ul>
                  <p  class="show-all" id="show-all" ></p>
                </div>
                <div style="display:none;">
                  ...
                  <p  class="show-less" id="show-less" style="display:none;" >Show less</p>
                </div>
          </section>
